...
function applyInlineIdCrop(){
 ...
 const img=new Image(); img.onload=()=>{state.processed=img;render()}; img.src=c.toDataURL('image/png');
}
...
function drawPhoto(it){
 ...
 const sourceImg=currentMode==='id-xerox'?(it.photoIndex%2===0?state.xeroxFront:state.xeroxBack):(((currentMode==='polaroid'||currentMode==='photo-booth')&&state.photos?.length)?state.photos[it.photoIndex%state.photos.length]:state.processed);
...
function drawRepeatedPreviewWatermark(){
 ctx.save();
 ctx.globalAlpha=0.22;
 ctx.fillStyle='#1f76d2';
 ctx.textAlign='center';
 ctx.textBaseline='middle';
 const stepX=Math.max(300,canvas.width/3.8);
 const stepY=Math.max(190,canvas.height/5.2);
 const mainSize=Math.max(56,Math.round(canvas.width*0.045));
 const subSize=Math.max(22,Math.round(canvas.width*0.018));
...
$('modeSelect').addEventListener('change',e=>{closeSharedCropUiForTabSwitch();currentMode=e.target.value;state=tabStates[currentMode];suspendInlineCropForInactiveTabs();const names={'id-photo':'EasyPhoto','polaroid':'EasyPhoto','photo-booth':'EasyPhoto','id-xerox':'EasyPhoto'};document.querySelector('.brand h1').textContent=names[currentMode]||'EasyPhoto';document.querySelector('.brand p').textContent='by PapaJekPrint';syncControlsFromState();});
document.addEventListener('input',e=>{if(['cols','rows'].includes(e.target.id))updateFormula(); if(e.target.id==='customQty'){const q=Math.max(1,+e.target.value||1); if(state.sizes[0]){state.sizes[0].qty=q;state.sizes[0].cols=1;state.sizes[0].rows=q;$('cols').value=1;$('rows').value=q;} updateFormula();refreshSizeList();render();return;} if(e.target.id==='zoom')state.zoom=+e.target.value; if(e.target.id==='brightness')state.brightness=+e.target.value; if(e.target.id==='contrast')state.contrast=+e.target.value; if(e.target.id==='margin')state.margin=+e.target.value; if(e.target.id==='spacing')state.spacing=+e.target.value; render();});
...
$('changePhotoBtn')?.addEventListener('click',e=>{e.preventDefault();state.inlineCropper?.destroy?.();state.inlineCropper=null;state.image=null;state.processed=null;state.imageUrl=null;$('thumb').hidden=true;$('thumb').removeAttribute('src');$('openCrop').disabled=true;updateIdPhotoUploadUi();$('fileInput').click();render();});
...
$('idSizeButtons')?.addEventListener('click',e=>{const btn=e.target.closest('[data-id-size]');if(!btn)return;const name=btn.dataset.idSize;const p=presetSets['id-photo'].items[name];$('photoW').value=p.w;$('photoH').value=p.h;$('unit').value=p.unit;document.querySelectorAll('[data-id-size]').forEach(b=>b.classList.toggle('active',b===btn));});
$('addQtySize').addEventListener('click',()=>{const qty=Math.max(1,+$('customQty').value||1);const active=document.querySelector('[data-id-size].active');const name=active?.dataset.idSize||'Added size';const w=+$('photoW').value||2;const h=+$('photoH').value||2;const unit=$('unit').value||'inch';state.sizes.push({name,w,h,unit,qty,cols:1,rows:qty});$('cols').value=1;$('rows').value=qty;updateFormula();refreshSizeList();render();});
...
$('downloadPng').onclick=()=>{render(false);downloadCanvas('id-photo-layout-300dpi.png');render(true);};
$('downloadPdf').onclick=()=>{const {jsPDF}=window.jspdf;let p={...papers[state.paper]};if(state.landscape&&p.h>p.w)[p.w,p.h]=[p.h,p.w];const pdf=new jsPDF({orientation:p.w>p.h?'l':'p',unit:'in',format:[p.w,p.h]});render(false);pdf.addImage(canvas.toDataURL('image/png'),'PNG',0,0,p.w,p.h);pdf.save('id-photo-layout.pdf');render(true)};
$('printBtn').onclick=()=>{render(false);const w=window.open('');w.document.write(`<img src="${canvas.toDataURL()}" style="width:100%;height:auto">`);w.document.close();w.print();render(true)};
