...
function updateDesignControls(){ if($('polaroidDesign')){ const selected=$('polaroidDesign').value; const showCustom=currentMode==='polaroid'&&(state.design==='custom'||selected==='custom'); if($('polaroidDesignLabel')) $('polaroidDesignLabel').style.display='block'; $('customBgLabel').hidden=false; $('customBgLabel').style.display=showCustom?'block':'none'; } if($('xeroxColorMode')){ $('xeroxColorMode').value=state.xeroxMode||'color'; document.querySelectorAll('[data-xerox-mode]').forEach(btn=>{const active=btn.dataset.xeroxMode===(state.xeroxMode||'color');btn.classList.toggle('primary',active);btn.classList.toggle('secondary',!active);}); $('xeroxFrontThumb').hidden=!state.xeroxFrontUrl; $('xeroxBackThumb').hidden=!state.xeroxBackUrl; if(state.xeroxFrontUrl)$('xeroxFrontThumb').src=state.xeroxFrontUrl; if(state.xeroxBackUrl)$('xeroxBackThumb').src=state.xeroxBackUrl; }}
...
$('addQtySize').addEventListener('click',()=>{const qty=Math.max(1,+$('customQty').value||1);const w=+$('photoW').value||2;const h=+$('photoH').value||2;const unit=$('unit').value||'inch';state.sizes.push({name:'Added size',w,h,unit,qty,cols:1,rows:qty});$('cols').value=1;$('rows').value=qty;updateFormula();refreshSizeList();render();});
